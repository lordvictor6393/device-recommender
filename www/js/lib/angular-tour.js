...
(function (window, document, undefined) {
    ...
    angular.module('angular-tour.tour', []).constant('tourConfig', {
        ...
        '$state',
        'tourConfig',
        function ($parse, $state, tourConfig) {
            return {
                controller: 'TourController',
                restrict: 'EA',
                scope: true,
                link: function (scope, element, attrs, ctrl) {
                    ...
                    scope.setCurrentStep = function (val, newState) {
                        console.log('redirect value: ' + newState);
                        if(newState !== 'none') $state.go(newState);
                    ...
                    scope.getRedirectState = function () {
                        var idx = scope.getCurrentStep() || 0;
                        return tourConfig.redirectTo[idx];
                    };
                }
            };
        }
        ...
        function ($window, $compile, $interpolate, $timeout, scrollTo, tourConfig) {
            ...
            return {
                ...
                link: function (scope, element, attrs, tourCtrl) {
                    ...
                    attrs.$observe('tourtipRedirectTo', function (val) {
                        scope.ttRedirectTo = val || 'none';
                        tourConfig.redirectTo[tourConfig.tourtips] = val;
                        tourConfig.tourtips++;
                    });
                    scope.ttOpen = false;
                    ...
                    function show() {
                        ...
                        var updatePosition = function () {
                            position = { top: targetElement.prop('offsetTop'), left: targetElement.prop('offsetLeft') };
        ...
        return function (target, offsetY, offsetX, speed) {
            if (target) {
                ...
                offsetX = offsetX || -100;
                speed = speed || 500;
                //angular.element(document.querySelector('html,body')).stop().animate({
                $('html, body').stop().animate({
                    scrollTop: target[0].getBoundingClientRect().top + offsetY,
                    scrollLeft: target[0].getBoundingClientRect().left + offsetX
                }, speed);
            } else {
                //angular.element(document.querySelector('html,body')).stop().animate({ scrollTop: 0 }, speed);
